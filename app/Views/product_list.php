<?= view('layout/header', ['title' => 'Product Inventory']) ?>

<div class="table-container">
    <h2>Product List</h2>
    <a href="<?= base_url('/product/create') ?>" class="btn-add" style="margin-bottom:15px;">Add Product</a>
    ...
    <table id="myTable" class="display" style="width:100%">
        ...
        <tbody>
            ...
            <tr data-id="<?= $p['id'] ?>">
                ...
                <td class="editable" data-field="price"><?= $p['price'] ?></td>
                <td class="editable" data-field="stock"><?= $p['stock'] ?></td>
                <td class="editable" data-field="description"><?= $p['description'] ?></td>
                <td>
    ...
    $(document).ready(function() {
        ...
        $('#myTable tbody').on('click', '.btn-edit', function() {
            ...
            tr.find('.editable').each(function() {
                ...
                var type = 'text';
                if(field === 'price' || field === 'stock') type = 'number';
        ...
        $('#myTable tbody').on('click', '.btn-save', function() {
            ...
            $.post('<?= base_url('/product/ajaxUpdate') ?>', updatedData, function(response) {
                if(response.status === 'success') {
                    ...
                    uniToast('Product updated successfully!');
                }
            }, 'json');
        });
        ...
        $('#myTable tbody').on('click', '.btn-delete', function() {
            ...
            uniConfirm('Are you sure you want to delete this product?', 'Confirm Deletion').then(function(confirmed) {
                if(!confirmed) return;
                
                $.post('<?= base_url('/product/ajaxDelete') ?>', {id: id}, function(response) {
                    if(response.status === 'success') {
